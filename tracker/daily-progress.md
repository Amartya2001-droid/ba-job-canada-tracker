...
- 2026-09-05: Recorded the real live URLs (GitHub repo and GitHub Pages dashboard) across every doc that referenced them only as placeholders — application pack, resume draft, proof checklist, send-ready package, cover notes, and all three LinkedIn drafts — with `docs/portfolio-links.md` as the canonical source. Documented the full 7-script SQL/SQLite pipeline in `docs/sqlite-runbook.md`, including two previously undocumented scripts and a flag on the legacy prototype pair that duplicates the current pipeline's output. Also hit and diagnosed a real git slowness incident on this machine (disk at 95% capacity); recorded safe handling steps in `docs/git-workflow-notes.md` rather than working around it with a verification bypass.
