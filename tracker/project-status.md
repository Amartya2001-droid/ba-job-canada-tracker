...
| Public Portfolio Deployment | Live | Pages is enabled and verified live at the URL in `docs/portfolio-links.md`; every deploy since 2026-08-16 has succeeded | Keep pushing to `main` to redeploy; no further setup needed |
...
| SQL/SQLite Pipeline | Documented | All 7 pipeline scripts are documented in `docs/sqlite-runbook.md`, including a flag on the legacy `build_wait_times_sqlite.sh`/`run_wait_times_reports.sh` pair that duplicates the current pipeline's output | Use the numbered steps in the runbook, not the legacy pair, for any future rebuild |
